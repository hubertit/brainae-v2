...
export default function LecturerPortalSidebar({ isOpen, onClose, onCollapsedChange }: LecturerPortalSidebarProps) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('lecturerSidebarCollapsed');
      if (saved !== null) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('lecturerSidebarCollapsed', collapsed.toString());
  ...
  const menuItems = [
    ...
    { icon: faEdit, label: 'Manage Assessments', href: '/lecturer/assessments' },
      ...
      >
          ...
          <div className={`flex items-center ${collapsed ? 'justify-center' : 'gap-2'}`}>
            ...
            {!collapsed && (
              <div className="flex items-center gap-2 flex-1">
                <h2 className="text-lg font-semibold text-gray-900">Lecturer Portal</h2>
                <button
        ...
        {userInfo && (
          ...
          `}>
            ...
            `}>
              <Icon icon={faChalkboardTeacher} className="text-gray-600" size={collapsed ? 'sm' : '2x'} />
            </div>
            {!collapsed && (
